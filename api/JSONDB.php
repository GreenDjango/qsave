...
class JSONDB {
	...
	private $order_by = [];
	private $max_limit = self::INF_LIMIT;
	protected $dir;
	private $json_opts = [];
		...
		if( !empty( $this->last_indexes ) && !empty( $this->where ) ) {
			foreach( $this->content as $i => $v ) {
				if( in_array( $i, $this->last_indexes ) ) {
					...
					if( 1 /* !array_diff_key( $this->update, $content ) */ ) {
...
';
	...
	public function limit( $limit = self::INF_LIMIT ) {
		$this->max_limit = $limit;
		return $this;
	}

	private function _process_limit( $content ) {
		if ( $this->max_limit != self::INF_LIMIT ) {
			$content = array_slice($content, 0, $this->max_limit);
		}
		return $content;
	}
	...
	public function get() {
		...
		$content = $this->_process_limit( $content );
